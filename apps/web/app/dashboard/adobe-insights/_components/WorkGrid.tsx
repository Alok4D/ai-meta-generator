            ...
            <div className={`grid gap-2 ${layoutMode === 'list' ? 'grid-cols-1' : layoutMode === 'half' ? 'grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3' : 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4'}`}>
                {works.map((item: any) => {
                    ...
                    return (
                        <div
                            key={item.id_work}
                            className={`group relative cursor-pointer border- transition-all ${layoutMode === 'list' ? 'flex flex-row h-48 bg-[#eaeaea] dark:bg-gray-800' : 'flex flex-col aspect-[4/3] bg-[#eaeaea] dark:bg-gray-800'} ${isSelected ? 'border-[#ffe169]' : 'border-transparent'}`}
                        >
                            {/* Image Container */}
                            <div
                            ...
                            >
                                ...
                                {isSelected && (
                                    <div className="absolute inset-0 border-[3px] border-[#ffe169] pointer-events-none z-10"></div>
                                )}
