...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Contributor Insights", href: "/contributor-insights" },
  ];
  ...
  return (
    <nav className="w-full bg-[#F3F5F6] sticky top-0 z-50 border-b border-gray-200/50 shadow-sm">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`md:text-[14px] lg:text-[15px] transition-colors duration-200 whitespace-nowrap ${isActive ? "text-gray-900 font-bold" : "text-slate-500 font-medium hover:text-slate-900"
                    }`}
                >
                  {link.name}
                </Link>
      ...
      <div
        className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${isMenuOpen ? "max-h-screen border-t border-gray-200/60" : "max-h-0"
          }`}
      >
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                href={link.href}
                className={`block px-4 py-3 rounded-xl text-[15px] font-medium transition-colors ${isActive
